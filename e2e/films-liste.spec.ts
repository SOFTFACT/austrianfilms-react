...
test('the list opens with every film, not only the current ones', async ({ page }) => {
  // Regression guard (2026-09-15): the list used to open with "current only",
  // and a title search then silently missed every film without an AF status.
  // The default view must show exactly what the API reports without a filter.
  const token = await page.evaluate(() => localStorage.getItem('austrianfilms_token'))
  const headers = { Authorization: `Bearer ${token}` }
  const all = await page.request.get('/api/v1/fmfilms?limit=1', { headers })
  const current = await page.request.get('/api/v1/fmfilms?actualOnly=true&limit=1', { headers })
  const allTotal = ((await all.json()) as { pagination: { total: number } }).pagination.total
  const currentTotal = ((await current.json()) as { pagination: { total: number } }).pagination.total
  // Counter-check: the two API totals must differ, or this test proves nothing.
  expect(allTotal).toBeGreaterThan(currentTotal)

  await expect.poll(async () => total(page), { timeout: 30_000 }).toBe(allTotal)
})

test('searching narrows the total', async ({ page }) => {
  const before = await total(page)

  // Ask the API for a title that is in the default (unfiltered) view and
  // search for that one, instead of relying on a fixed term.
  const token = await page.evaluate(() => localStorage.getItem('austrianfilms_token'))
  const res = await page.request.get('/api/v1/fmfilms?limit=1', {
